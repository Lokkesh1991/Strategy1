    ...
    while True:
        ...
        if current_lots < LOTS_TO_SELL:
            ...
            while oi < len(exit_strikes) and ei < len(enter_strikes):
                if exit_strikes[oi][0] not in available_strikes:
                    print(f"⚠️ Skipping exit for unavailable strike: {exit_strikes[oi][0]}")
                    oi += 1
                    continue
                if enter_strikes[ei][0] not in available_strikes:
                    print(f"⚠️ Skipping entry for unavailable strike: {enter_strikes[ei][0]}")
                    ei += 1
                    continue
